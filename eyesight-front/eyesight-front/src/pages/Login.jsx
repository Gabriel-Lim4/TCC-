...
//   1. Salva "usuario" no localStorage após login/cadastro
//      (necessário para o fluxo OAuth do Meta)
//   2. Corrige leitura do erro: back retorna "erro", não "error"/"message"
//   3. Após cadastro bem-sucedido, mantém o e-mail preenchido
//      para facilitar o login logo em seguida
// ─────────────────────────────────────────────────────────────
...
function Login() {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (data.token) {
        ...
        // Salva o objeto usuario — usado no fluxo OAuth do Meta
        // para identificar o usuário no callback sem JWT
        localStorage.setItem('usuario', JSON.stringify(data.usuario));
      }

      navigate('/app/dashboard');
    } catch (err) {
      // O back-end retorna o campo "erro" (não "error" nem "message")
  ...
  async function handleCadastro() {
    ...
    try {
      ...
      // Após cadastrar, volta para o login com e-mail preenchido
    ...
    <div className={`login-container ${isCadastro ? 'cadastro-mode' : ''}`}>
      ...
      <div className="right-side">
        <div className="login-box">
          ...
          {sucesso && <p className="erro-msg" style={{ color: '#38a169' }}>{sucesso}</p>}
