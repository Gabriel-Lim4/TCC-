...
// metaController.js
export async function resumoInsights(req, res) {
  ...
  try {
    ...
    if (!conta) return res.status(404).json({ erro: 'Conta não vinculada.' });

    const campanhas = await prisma.campanhas.findMany({
      where:  { fk_id_conta: conta.pk_id_conta },
      select: { pk_id_campanha: true, id_externo: true },
    });

    const resultados = await Promise.allSettled(
      campanhas.map(c => metaService.buscarInsights(c.id_externo, conta.acess_token, periodo))
    );

    const totais = { impressions: 0, clicks: 0, spend: 0, reach: 0, ctr: 0, cpm: 0, frequency: 0 };
    let count = 0;
    for (const r of resultados) {
      if (r.status === 'fulfilled' && r.value) {
        totais.impressions += parseFloat(r.value.impressions || 0);
        totais.clicks      += parseFloat(r.value.clicks || 0);
        totais.spend       += parseFloat(r.value.spend || 0);
        totais.reach       += parseFloat(r.value.reach || 0);
        totais.ctr         += parseFloat(r.value.ctr || 0);
        totais.cpm         += parseFloat(r.value.cpm || 0);
        totais.frequency   += parseFloat(r.value.frequency || 0);
        count++;
      }
    }
    if (count > 0) { totais.ctr /= count; totais.cpm /= count; totais.frequency /= count; }

    return res.status(200).json({ periodo, totais, campanhas_com_dados: count });

  } catch (err) {
    console.error('[META] Erro ao buscar resumo:', err.message);
    return res.status(500).json({ erro: 'Erro interno ao buscar resumo.' });
  }
}